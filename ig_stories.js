...
let seeReels = () => {
  ...
  let imgHtml = images.reverse().map(i => '<a href="' + i + '" target="_blank"><img height="400" src="' + i + '"/></a>').join('');
  let vidHtml = videos.reverse().map(i => '<div style="width: max-content;"><a href="' + i + '" target="_blank" >' + i + '</a></div>').join('');
